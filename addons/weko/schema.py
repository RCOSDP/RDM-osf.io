...
def _get_metadata_value(file_metadata_data, item, lang, index):
    ...
    if item['type'] == 'const':
        if 'depends' in item:
            value = _get_metadata_value(
                file_metadata_data, item['depends'], lang, index
            )
            if not value:
                return ''
        return item['value']
    ...
    if 'value' in file_metadata:
        ...
        if schema is not None and 'options' in schema:
            options = [
                ...
                if o.get('text', None) == v
                or (not v and o.get('default', False))
            ]
            if len(options) == 0:
                logger.debug(
                    f'No suitable options: value={v}, schema={schema}'
                )
            else:
                option = options[0]
                v = option['text']
...
def _is_column_present(file_metadata, item, commonvars=None, schema=None):
    ...
    if present_expression is None:
        return True
    value = _get_value(
        file_metadata, present_expression, commonvars=commonvars, schema=schema
    )
...
def _get_columns(
    ...
    weko_key_prefix,
    weko_props,
    weko_key_counts=None,
    commonvars=None,
    schema=None,
):
    if isinstance(weko_props, str):
        value = _get_value(
            file_metadata, weko_props, commonvars=commonvars, schema=schema
        )
        return _to_columns(
            weko_key_prefix, value, weko_key_counts=weko_key_counts
        )
    columns = []
    for key in sorted(weko_props.keys()):
        ...
        for item in items:
            if not _is_column_present(
                file_metadata, item, commonvars=commonvars, schema=schema
            ):
                continue
            full_key = _resolve_array_index(
                weko_key_counts, f'{weko_key_prefix}.{key}'
            )
            ...
            columns += _to_columns(
                full_key, value, weko_key_counts=weko_key_counts
            )
    return columns
...
def get_available_schema_id(file_metadata):
    ...
    items = [
        ...
        and item.get('schema', None) in available_schema_ids
    ]
    if len(items):
...
def _resolve_array_index(weko_key_counts, key):
    ...
    matched = [
        i
        for i, weko_key_id in enumerate(weko_key_ids)
        if weko_key_id == index_id
    ]
    if len(matched) == 0:
        # New key
        weko_key_count = len(weko_key_ids)
...
def _concatenate_sources(metadatas, check_duplicates=[]):
    ...
    for key in keys:
        ...
        if len(items) == 0:
            continue
        empty_items = [
            item for item in items if _is_empty(item.get('value', ''))
        ]
        non_empty_items = [
            item for item in items if not _is_empty(item.get('value', ''))
        ]
        ...
        raise ValueError(
            f'Duplicated values for key: {key}, {non_empty_values}'
        )
    return r
...
def _get_sources_for_key(
    ...
    common_file_metadata_data = _concatenate_sources(
        common_file_metadata_datas
    )
    ...
    if key == '@files':
        if len(file_metadatas) != len(download_file_names):
            raise ValueError(
                f'File metadata count mismatch: {len(file_metadatas)} != {len(download_file_names)}'
            )
        r = []
        for file_metadata, (download_file_name, download_file_type) in zip(
            file_metadatas, download_file_names
        ):
            file_metadata_items = [
                item
                for item in file_metadata['items']
            ...
            if len(file_metadata_items) == 0:
                raise ValueError(
                    f'Schema not found: {file_metadata}, {schema._id}'
                )
...
def write_csv(
    ...
    header = [
        '#ItemType',
        itemtype_metadata['name'],
        itemtype_metadata['schema'],
    ]
    ...
    for i, (download_file_name, download_file_type) in enumerate(
        download_file_names
    ):
        columns += _generate_file_columns(
            i, download_file_name, download_file_type
        )
    ...
    for key in sorted(mappings.keys()):
        for source, commonvars in _get_sources_for_key(
            ...
            question_schema = (
                _find_schema_question(schema.schema, key)
                if not _is_special_key(key)
                else None
            )
