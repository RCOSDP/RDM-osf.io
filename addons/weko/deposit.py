...
def _deposit_metadata(
    ...
    status_path,
    delete_after=False,
    delete_temp_dir_immediately=True,
    ...
    try:
            ...
            if 'choose-additional-metadata' in metadata:
                try:
                    file_list = json.loads(
                        metadata['choose-additional-metadata']['value']
                    )
                    ad_metadata_paths.extend(
                        item['path'] for item in file_list
                    )
        ...
        for path in ad_metadata_paths:
            ...
            if file is None:
                raise KeyError(f'File not found: {materialized_path}')
            download_file_path, download_file_type = _download(
                node, file, tmp_dir, ad_metadata_total_size
            )
            filesize = os.path.getsize(download_file_path)
            ad_metadata_total_size += filesize
            _, download_file_name = os.path.split(download_file_path)
            ad_metadata_download_file_names.append(
                (download_file_name, download_file_type)
            )
        ...
        if (
            ...
            and user.affiliated_institutions.first()
        ):
            bagit_metadata['Source-Organization'] = (
                user.affiliated_institutions.first().name
            )
        bag = bagit.make_bag(bagit_dir, bagit_metadata)

        for download_file_name, _ in download_file_names:
            file_in_bagit_path = os.path.join(
                bagit_dir, 'data', 'files', download_file_name
            )
            os.makedirs(os.path.dirname(file_in_bagit_path), exist_ok=True)
            shutil.copyfile(
                os.path.join(tmp_dir, download_file_name), file_in_bagit_path
            )
        ...
        if mapping_def_csv is not None:
            with open(
                os.path.join(bagit_dir, 'data', 'index.csv'),
                'w',
                encoding='utf8',
            ) as f:
                schema.write_csv(
        ...
        if mapping_def_ro_crate_json is not None:
            with open(
                os.path.join(bagit_dir, 'data', 'ro-crate-metadata.json'),
                'w',
                encoding='utf8',
            ) as f:
        ...
        with ZipFile(zip_path, 'w') as zipf:
            for root, dirs, files in os.walk(bagit_dir):
                for file in files:
                    ...
                    zipf.write(
                        file_path, os.path.relpath(file_path, bagit_dir)
                    )
        ...
        if (
            delete_temp_dir_immediately
            and bagit_dir
            and os.path.exists(bagit_dir)
        ):
