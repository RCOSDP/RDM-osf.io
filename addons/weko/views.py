...
def _response_file_metadata(
    addon, path, progress=None, result=None, error=None
):
    attr = {
        'path': path,
    }
...
def _response_project_metadata(
    addon, metadata_type, metadata_id, progress=None, result=None, error=None
):
    attr = {
...
def weko_publish_file(
    auth, did=None, index_id=None, mnode=None, filepath=None, **kwargs
):
    node = kwargs['node'] or kwargs['project']
    ...
    if not addon.validate_index_id(index_id):
        ...
        return HTTPError(http_status.HTTP_400_BAD_REQUEST)
    schema_id = (
        request.json.get('schema_id', None)
    ...
    project_metadata_ids_text = (
        request.json.get('project_metadata_ids', None)
        if request.json is not None
        else None
    )
    project_metadata_ids = (
        project_metadata_ids_text.split(',')
        if project_metadata_ids_text is not None
        else []
    )
    project_metadatas = []
    for project_metadata_id in project_metadata_ids:
        ...
        if len(project_metadata_id) == 0:
            continue
        if project_metadata_id is not None and project_metadata_id.startswith(
            'registration/'
        ):
            id = project_metadata_id[len('registration/') :]
            project_metadata = Registration.objects.filter(
                guids___id=id
            ).first()
            project_metadata_ = project_metadata.registered_meta[schema_id]
        elif (
            project_metadata_id is not None
            and project_metadata_id.startswith('draft-registration/')
        ):
...
def weko_get_publishing_file(
    ...
    return _response_file_metadata(
        addon, filepath, progress=progress, error=error, result=result
    )


def _publish_project_metadata(
    auth,
    node,
    addon,
    index_id,
...
def _get_publishing_project_metadata_progress(
    addon, metadata_type, metadata_id
):
    status_path = f'/_/{metadata_type}/{metadata_id}'
    ...
    if aresult.failed():
        ...
        result = aresult.info['result']
    return _response_project_metadata(
        addon,
        metadata_type,
        metadata_id,
        progress=progress,
...
@must_have_addon(SHORT_NAME, 'node')
@must_have_addon(METADATA_SHORT_NAME, 'node')
def weko_publish_registration(
    auth, did=None, index_id=None, registration_id=None, **kwargs
):
    node = kwargs['node'] or kwargs['project']
    addon = node.get_addon(SHORT_NAME)
    project_metadata = Registration.objects.filter(
...
@must_have_addon(SHORT_NAME, 'node')
@must_have_addon(METADATA_SHORT_NAME, 'node')
def weko_get_publishing_registration(
    auth, did=None, index_id=None, registration_id=None, **kwargs
):
    node = kwargs['node'] or kwargs['project']
    addon = node.get_addon(SHORT_NAME)
    return _get_publishing_project_metadata_progress(
        addon, 'registration', registration_id
    )
...
@must_have_addon(SHORT_NAME, 'node')
@must_have_addon(METADATA_SHORT_NAME, 'node')
def weko_publish_draft_registration(
    auth, did=None, index_id=None, draft_registration_id=None, **kwargs
):
    node = kwargs['node'] or kwargs['project']
    addon = node.get_addon(SHORT_NAME)
    project_metadata = DraftRegistration.objects.filter(
        _id=draft_registration_id
    ).first()
    project_metadata_ = project_metadata.registration_metadata
...
@must_have_addon(SHORT_NAME, 'node')
@must_have_addon(METADATA_SHORT_NAME, 'node')
def weko_get_publishing_draft_registration(
    auth, did=None, index_id=None, draft_registration_id=None, **kwargs
):
    node = kwargs['node'] or kwargs['project']
    addon = node.get_addon(SHORT_NAME)
    return _get_publishing_project_metadata_progress(
        addon, 'draft_registration', draft_registration_id
    )
...
def weko_get_available_schemas(auth, **kwargs):
    ...
    addon = node.get_addon(SHORT_NAME)
    schemas = [
        _response_registration_schema(
            RegistrationSchema.objects.get(_id=schema_id)
        )
