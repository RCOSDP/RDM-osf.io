/**
 * Module that controls the Groups node settings. Includes Knockout view-model
 * for syncing data.
 */
...
const logPrefix = '[groups]';

const $modal = $('#groupsApplyDialog');
...
ViewModel.prototype.refresh = function(callback) {
    ...
    $.ajax({
        ...
        self.changeMessage(_('Could not GET groups settings'), 'text-danger');
        Raven.captureMessage('Could not GET groups settings', {
            extra: {
                url: self.url,
...
function GroupsNodeConfig(selector, nodeId, url) {
    // Initialization code
    const self = this;
    self.nodeId = nodeId;
    self.url = url;
...
module.exports = GroupsNodeConfig;
