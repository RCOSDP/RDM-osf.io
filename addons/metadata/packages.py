...
class WaterButlerClientForNode(object):
    ...
    def upload_root_file(self, file, file_name, provider):
        resp = waterbutler.upload_file(
            self.cookie, self.node._id, file, file_name, provider
        )
        data = resp.json()['data']
        return WaterButlerObject(data, self)
    ...
    def get_file_by_materialized_path(self, path, create=False):
        ...
        if len(path_segments) == 2:
            ...
            if folder:
                target_path += '/'
            logger.debug(
                f'Fetching... provider={provider}, path={target_path}'
            )
            files = self.get_root_files(provider)
            logger.debug(
                f'Fetched: provider={provider}, path={target_path}, files={files}'
            )
            candidates = [
                file
                for file in files
                if 'materialized' in file.attributes
                and file.attributes['materialized'] == f'/{target_path}'
            ]
        ...
        parent_file = self.get_file_by_materialized_path(
            parent_path, create=create
        )
        target_path = '/'.join(path_segments[1:])
        if folder:
            target_path += '/'
...
class BaseROCrateFactory(object):
    ...
    def _ro_crate_path_list(self):
        ...
        for path, file, _ in files:
            ...
            total_size += size
            logger.info(
                f'Downloaded: path={path}, size={size} (total downloaded={total_size})'
            )
            yield {
                'fs': tmp_path,
                'n': path[2:],
    ...
    def _create_file_entities(
        ...
        if wb_file.attributes['kind'] == 'folder':
            ...
            for child in wb_files:
                _, children = self._create_file_entities(
                    crate, node, path, child, user_ids, schema_ids, comment_ids
                )
                r += children
            entity = crate.add(
                DataEntity(
                    ...
                    properties={
                        ...
                        'hasPart': [
                            {
                                '@id': f'{path}{child.name}/'
                                if child.attributes['kind'] == 'folder'
                                else f'{path[2:]}{child.name}'
                            }
                            for child in wb_files
                        ],
                    },
            ...
            if web_file is not None:
                ...
                if self.include_users and creator._id not in user_ids:
                    crate.add(
                        *self._create_contributor_entities(
                            crate, creator, user_ids
                        )
                    )
                comments = sum(
                    [
                        self._create_comment_entities(
                            crate, path, None, c, user_ids, comment_ids
                        )
        ...
        self._create_file_metadata_entities(
            crate, node, path, self._get_materialized_path(wb_file), schema_ids
        )
        return entity, r
    ...
    def _get_materialized_path(self, wb_file):
        ...
        return (
            wb_file.attributes['provider'] + wb_file.attributes['materialized']
        )

    def _create_project_entities(
        self, crate, entity_id, node, user_ids, extra_props=None
    ):
        props = {
            '@type': 'RDMProject',
            'name': node.title,
        ...
        license = ContextEntity(
            ...
            properties={
                ...
                'rdmLicenseProperties': list(
                    node.license.node_license.properties
                ),
            },
        )
        props['license'] = {
    ...
    def _create_project_metadata_entities(
        self,
        crate,
        node,
        draft_or_registration,
        node_ids,
        schema_ids,
        project_metadata_ids,
    ):
        metadata_props, new_schema_ids = (
            convert_project_metadata_to_json_ld_entities(draft_or_registration)
        )
        ...
        crate.add(
            ContextEntity(
                crate, project_metadata_id, properties=metadata_props
            )
        )
    ...
    def _create_file_metadata_entities(
        self, crate, node, path, file_path, schema_ids
    ):
        metadata = node.get_addon(SHORT_NAME)
        if metadata is None:
            return
        file_metadata = metadata.get_file_metadata_for_path(
            file_path, resolve_parent=False
        )
        ...
        file_metadata_props, new_schema_ids = (
            convert_file_metadata_to_json_ld_entities(file_metadata)
        )
        for i, entity in enumerate(file_metadata_props):
            ...
            crate.add(
                ContextEntity(crate, f'{file_entity_id}#{i}', properties=props)
            )
        for schema_id, schema_props in new_schema_ids.items():
            if schema_id in schema_ids:
                continue
    ...
    def _create_comment_entities(
        self, crate, about_id, reply_for_id, comment, user_ids, comment_ids
    ):
        if self.include_users and comment.user._id not in user_ids:
            crate.add(
                *self._create_contributor_entities(
                    crate, comment.user, user_ids
                )
            )
        ...
        props = {
            ...
            'author': {'@id': user_ids[comment.user._id]}
            if comment.user._id in user_ids
            else None,
        }
        ...
        for reply in Comment.objects.filter(
            target___id=comment._id, deleted__isnull=True
        ):
            r += self._create_comment_entities(
                crate, None, comment_id, reply, user_ids, comment_ids
            )
        return r
    ...
    def _create_person_entities(self, crate, user, entity_id, user_ids):
        ...
        institution_id = (
            f'#{affiliation["institution"] or affiliation["institution_ja"]}'
        )
        ...
        if not (affiliation['department'] or affiliation['department_ja']):
            return [
                Person(crate, entity_id, properties=person_props),
                ContextEntity(
                    crate, institution_id, properties=institution_props
                ),
            ]
        department_name = []
    ...
    def _create_log_entity(self, crate, log, user_ids, action_ids):
        ...
        return ContextEntity(
            ...
            properties={
                ...
                'agent': {'@id': user_ids[log.user._id]}
                if log.user is not None and log.user._id in user_ids
                else None,
            },
        )
    ...
    def _create_addon_entity(self, crate, node_id, addon, extra_props=None):
        ...
        if extra_props is not None:
            props.update(extra_props)
        return ContextEntity(
            crate, f'{node_id}-{addon.config.short_name}', properties=props
        )

    def _create_wiki_entities(
        self, crate, base_path, wiki, user_ids, comment_ids
    ):
        r = []
        ...
        comments = sum(
            [
                ...
                for c in Comment.objects.filter(
                    root_target=Guid.load(wiki._id), deleted__isnull=True
                )
            ],
            [],
        )
        ...
        props = {
            ...
            'contentSize': str(len(latest.content))
            if latest is not None
            else None,
...
class ROCrateFactory(BaseROCrateFactory):
    ...
    def _create_comment_entities(
        self, crate, parent_id, reply_for_id, comment, user_ids, comment_ids
    ):
        comment_config = self.config.get('comment', {})
        if not comment_config.get('enable', True):
            return []
        return super()._create_comment_entities(
            crate, parent_id, reply_for_id, comment, user_ids, comment_ids
        )
    ...
    def _is_file_archivable(self, wb_file):
        ...
        logger.debug(
            f'is_file_archivable: {materialized}, provider={provider}, files={addons_config}'
        )
        ...
        files = [f for f in files if f['materialized'] == materialized]
        if len(files) == 0:
            return True
    ...
    def _create_project_related_entities(
        ...
        for child in node.nodes:
            ...
            child_entities_, child_files_ = (
                self._create_project_related_entities(
                    crate,
                    base_file_prefix,
                    child,
                    user_ids,
        ...
        entities += sum(
            [
                self._create_comment_entities(
                    crate, entity_id, None, comment, user_ids, comment_ids
                )
                for comment in Comment.objects.filter(
                    root_target=Guid.load(node._id), deleted__isnull=True
                )
            ],
            [],
        )
        ...
        for addon_app in website_settings.ADDONS_AVAILABLE:
            ...
            if addon is None:
                continue
            if addon_name in addons_config and not addons_config[
                addon_name
            ].get('enable', True):
                logger.info(f'Skipped {addon_name}')
                crate.add(
                    self._create_addon_entity(
                        crate,
                        entity_id,
                        addon,
                    )
                ...
                continue
            if not (
                hasattr(addon, 'serialize_waterbutler_credentials')
                and addon.complete
            ):
                crate.add(
                    self._create_addon_entity(
            ...
            for file in wb.get_root_files(addon_name):
                entity, children = self._create_file_entities(
                    crate,
                    node,
                    f'{file_prefix}{addon_name}',
                    file,
                    user_ids,
                    schema_ids,
                    comment_ids,
        ...
        for draft in drafts.all():
            self._create_project_metadata_entities(
                crate, node, draft, node_ids, schema_ids, project_metadata_ids
            )
        ...
        if wiki_config.get('enable', True):
            ...
            for wiki in node.wikis.filter(deleted__isnull=True):
                wikis += self._create_wiki_entities(
                    crate, f'{file_prefix}wiki/', wiki, user_ids, comment_ids
                )
            crate.add(
                self._create_addon_entity(
                    crate,
        ...
        if log_config.get('enable', True):
            for log in node.logs.all():
                entity = self._create_log_entity(
                    crate, log, user_ids, action_ids
                )
                if entity is None:
                    continue
    ...
    def _build_ro_crate(self, crate):
        ...
        for node in [self.node] + list(self.node.get_descendants_recursive()):
            creators += self._create_creator_entities(
                crate, node.creator, user_ids
            )
            contributors += sum(
                [
                    self._create_contributor_entities(crate, user, user_ids)
                    for user in node.contributors.all()
                ],
                [],
            )
        ...
        action_ids = {}
        project_metadata_ids = {}
        project_entities, project_files = (
            self._create_project_related_entities(
                crate,
                './',
                self.node,
...
class ROCrateExtractor(object):
    ...
    def ensure_node(self, node, node_id='#root'):
        ...
        # tags
        for keyword in (
            self.crate.get(node_id).properties().get('keywords', [])
        ):
            self._ensure_tag(node, keyword)

        # restore addons
        for addon in self._find_related_entities(
            node_id, lambda e: e.type == 'RDMAddon'
        ):
    ...
    def _ensure_addon(self, node, addon):
        ...
        if addon_object is None:
            addon_object = node.add_addon(
                addon_name, auth=Auth(user=self.user), log=False
            )
        folder_id = addon.properties().get('rdmFolderId', None)
        if not folder_id:
            return
        metadata_addon = node.get_or_add_addon(
            METADATA_SHORT_NAME, auth=Auth(user=self.user)
        )
        metadata_addon.add_imported_addon_settings(addon_name, folder_id)

    def ensure_folders(self, wb):
        addons = [e for e in self.crate.get_entities() if e.type == 'RDMAddon']
        for addon_entity in addons:
            node = self.related_nodes[
                addon_entity.properties()['about']['@id']
            ]
    ...
    def _ensure_folders(self, wb, node, base_path, folder):
        ...
        wb.get_client_for_node(node).get_file_by_materialized_path(
            path, create=True
        )
    ...
    def _find_related_entities(self, about_id, filter):
        return [
            ...
            if e.properties().get('about', {}).get('@id', None) == about_id
            and filter(e)
        ]
        ...
        with ZipFile(self.zip_path, 'r') as zf:
            for i, file_name in enumerate(zf.namelist()):
                ...
                archive_path = os.path.join(
                    *[
                        e.properties()['name']
                        for e in self._get_path_for_file(file_name)
                    ]
                )
    ...
    def _find_parent_entities_for_file(self, file_path):
        return [
            ...
            in [
                part.get('@id', None)
                for part in e.properties().get('hasPart', [])
            ]
        ]
    ...
    def _download(self, file):
        ...
        if resp.status_code == 200 and 'login' in resp.url and resp_is_html:
            raise PermissionsError(
                f'URL {self.url} is not publicly accessible'
            )
    ...
    def _ensure_file_metadata(self, node, entity, file_path):
        metadata_entities = [
            e
            for e in self.crate.get_entities()
            if e.properties().get('about', {}).get('@id', None) == entity.id
            and e.type == 'RDMFileMetadata'
        ]
        if len(metadata_entities) == 0:
            return
        logger.debug(
            f'Metadata for {entity.id}({file_path}) = {metadata_entities}'
        )
        ...
        items = [
            convert_json_ld_entity_to_file_metadata_item(
                e.properties(), self._crate
            )
            for e in metadata_entities
        ]
        metadata_addon.set_file_metadata(
            file_path,
            {
                'path': file_path,
...
class FileExtractor(BaseExtractor):
    ...
    def extract(self, wb_client):
        ...
        self.owner._ensure_file_metadata(
            self.node, self.entity, self.file_path
        )
...
def to_creators_metadata(users):
    return [_to_user_metadata(user) for user in users]
...
def _to_localized(o, prop, default_lang='en'):
    ...
    items.append(
        dict(
            ...
            + ([('@language', default_lang)] if default_lang else [])
        )
    )
    prop_ja = f'{prop}_ja'
...
def _to_i18n_metadata(
    prefix, object, names, languages=['ja', 'en'], get_value=getattr
):
    r = {}
...
def export_project(self, user_id, node_id, config):
    ...
    metadata_addon = node.get_addon(SHORT_NAME)
    schema_id = (
        RegistrationSchema.objects.filter(name=EXPORT_REGISTRATION_SCHEMA_NAME)
    ...
    try:
        ...
        uploaded = wb.get_client_for_node(node).upload_root_file(
            zip_path, file_name_, provider_name
        )
        default_data = {
            'grdm-file:title-ja': to_metadata_value(node.title),
            'grdm-file:data-description-ja': to_metadata_value(
                node.description
            ),
            'grdm-file:creators': to_metadata_value(
                to_creators_metadata([user] if user is not None else [])
            ),
        ...
        if node.node_license is not None:
            default_data['grdm-file:data-policy-license'] = to_metadata_value(
                node.node_license.license_id
            )
        ...
        if len(current_jobs) > 0:
            default_data.update(
                _to_i18n_metadata(
                    ...
                    get_value=lambda o, k, default: html.unescape(o[k])
                    if k in o
                    else default,
                )
            )
        ...
        if len(institutions) > 0:
            default_data['grdm-file:hosting-inst-ja'] = to_metadata_value(
                institutions[0].name
            )
        metadata = {
    ...
    if result.failed():
        ...
        if 'node' in result.info:
            ...
            if 'file' in result.info:
                ...
                file_url = node.web_url_for(
                    'addon_view_or_download_file', path=path, provider=provider
                )
