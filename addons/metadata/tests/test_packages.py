...
def _create_waterbutler_node_client(new_node, files):
    def _get_root_files(name):
        ...
        if name not in files:
            return []
        return [_create_waterbutler_object(name, file) for file in files[name]]
    ...
    def _get_file_by_materialized_path(path, create=False):
        def _upload_file(data_path, dest_name):
            ...
            root_file = (
                new_node.get_addon(provider).get_root().append_file(dest_name)
            )
            r.provider = provider
    ...
    wb.get_file_by_materialized_path = mock.MagicMock(
        side_effect=_get_file_by_materialized_path
    )
    return wb, mock_upload_file


def _create_waterbutler_client_for_single_node(new_node, files):
    node_wb, node_wb_upload_file = _create_waterbutler_node_client(
        new_node, files
    )
    wb = mock.MagicMock()
    ...
    for k, v in expected_with_patterns.items():
        ...
        if isinstance(v, dict):
            ...
            assert_equals(
                len(value[k]),
                len(v),
                f'Length mismatch: {len(value[k])} != {len(v)} (expected {v})',
            )
            ...
            assert_true(
                v.match(value[k]),
                f'Unexpected value: {value[k]} (expected {v})',
            )
        else:
            assert_equals(
                value[k], v, f'Unexpected value: {value[k]} (expected {v})'
            )
...
class TestExportAndImport(OsfTestCase):
    def setUp(self):
        ...
        self.node.add_tags(
            ['Test Node'], auth=Auth(user=self.node.creator), log=False
        )
        self.node.save()
        self.node_comment_1 = CommentFactory(
            node=self.node, user=self.node.creator
        )
        ...
        root_file = (
            self.node.get_addon('osfstorage')
            .get_root()
            .append_file('file_in_root')
        )
        ...
        file = (
            self.node.get_addon('osfstorage')
            .get_root()
            .append_file('file_in_folder')
        )
        file.add_tags(
            ['Test File'], auth=Auth(user=self.node.creator), log=False
        )
        file.save()
        sub_file = (
            self.node.get_addon('osfstorage')
            .get_root()
            .append_file('file_in_sub_folder')
        )

        _, node_wb, _ = _create_waterbutler_client_for_single_node(
            self.node,
            {
                'osfstorage': [
                    {
        ...
        self.node.get_addon('metadata').set_file_metadata(
            'osfstorage/file_in_root',
            {
        ...
        self.composite_node.add_tags(
            ['Composite Node'], auth=Auth(user=self.node.creator), log=False
        )
        self.composite_node.save()
        root_file = (
            self.composite_node.get_addon('osfstorage')
            .get_root()
            .append_file('file_in_composite_root')
        )
        root_file_comment = CommentFactory(
        ...
        root_file_comment.content = (
            'Comment for the file file_in_composite_root'
        )
        root_file_comment.save()

        self.composite_node.get_addon('metadata').set_file_metadata(
            'osfstorage/file_in_composite_root',
            {
        ...
        _, composite_node_wb, _ = _create_waterbutler_client_for_single_node(
            self.composite_node,
            {
                'osfstorage': [
                    {
        ...
        self.sub_node = ProjectFactory(
            parent=self.composite_node, creator=self.composite_node.creator
        )
        self.sub_node.description = 'TEST_SUB_DESCRIPTION'
        self.sub_node.add_tags(
            ['Sub Node'], auth=Auth(user=self.node.creator), log=False
        )
        self.sub_node.save()
        root_file = (
            self.sub_node.get_addon('osfstorage')
            .get_root()
            .append_file('file_in_sub_root')
        )
        root_file_comment = CommentFactory(
            node=self.sub_node,
        ...
        root_file_comment.save()
        _, sub_node_wb, _ = _create_waterbutler_client_for_single_node(
            self.sub_node,
            {
                'osfstorage': [
                    {
        ...
        self.wb.get_client_for_node = mock.MagicMock(
            side_effect=get_wb_client_for_node
        )
    ...
    def test_files_only(self):
        ...
        assert_equals(
            remove_fields(
                _find_entity_by_id(json_entities, './'),
        ...
        assert_equals(
            remove_fields(
                _find_entity_by_id(
                    json_entities, 'root/osfstorage/file_in_root'
                ),
                fields=['rdmURL'],
            ),
            {
        ...
        assert_equals(
            remove_fields(
                _find_entity_by_id(
                    json_entities, '#root/osfstorage/file_in_root#0'
                ),
            ...
            ),
            {
                ...
                'about': {'@id': 'root/osfstorage/file_in_root'},
        ...
        assert_equals(
            _find_entity_by_id(json_entities, 'ro-crate-metadata.json'),
            {
        ...
        assert_equals(
            _find_entity_by_id(json_entities, '#root-osfstorage'),
            {
                '@id': '#root-osfstorage',
                '@type': 'RDMAddon',
                'about': {'@id': '#root'},
        ...
        assert_equals(
            _find_entity_by_id(json_entities, '#root-metadata'),
            {
                '@id': '#root-metadata',
                '@type': 'RDMAddon',
                'about': {'@id': '#root'},
        ...
        assert_true(
            'dateCreated' in _find_entity_by_id(json_entities, '#root')
        )
        assert_true(
            'dateModified' in _find_entity_by_id(json_entities, '#root')
        )
        assert_true('name' in _find_entity_by_id(json_entities, '#root'))
        creator = self.node.creator
        assert_equals(
            _find_entity_by_id(json_entities, '#creator0'),
            {
                '@id': '#creator0',
                '@type': 'Person',
                'familyName': [
                    {'@language': 'en', '@value': creator.family_name}
                ],
                'givenName': [
                    {'@language': 'en', '@value': creator.given_name}
                ],
                'identifier': [],
    ...
    def test_comments_and_files_only(self):
        ...
        _assert_dict_matches(
            _find_entity_by_id(json_entities, '#creator0'),
            {
                ...
                '@type': 'Person',
                'familyName': [
                    {'@language': 'en', '@value': re.compile(r'Mercury[0-9]+')}
                ],
                'givenName': [{'@language': 'en', '@value': 'Freddie'}],
                'identifier': [],
        ...
        assert_true(
            'dateCreated' in _find_entity_by_id(json_entities, '#comment#0')
        )
        assert_true(
            'dateModified' in _find_entity_by_id(json_entities, '#comment#0')
        )
        assert_equals(
            remove_fields(
        ...
        assert_true(
            'dateCreated' in _find_entity_by_id(json_entities, '#comment#1')
        )
        assert_true(
            'dateModified' in _find_entity_by_id(json_entities, '#comment#1')
        )
        assert_equals(
            remove_fields(
        ...
        assert_true(
            'dateCreated' in _find_entity_by_id(json_entities, '#comment#2')
        )
        assert_true(
            'dateModified' in _find_entity_by_id(json_entities, '#comment#2')
        )
    ...
    def test_wiki_only(self):
        ...
        assert_equals(
            len(
                [e for e in json_entities['@graph'] if e['@type'] == 'Action']
            ),
            0,
        )
        assert_equals(
        ...
        assert_true(
            'dateCreated'
            in _find_entity_by_id(json_entities, 'root/wiki/test')
        )
        assert_true(
            'dateModified'
            in _find_entity_by_id(json_entities, 'root/wiki/test')
        ...
        assert_equals(
            _find_entity_by_id(json_entities, '#root-wiki'),
            {
                '@id': '#root-wiki',
                '@type': 'RDMAddon',
                'about': {'@id': '#root'},
    ...
    def test_child_nodes(self):
        ...
        rocrate = ROCrateFactory(
            self.composite_node, self.work_dir, self.wb, config
        )
        zip_path = os.path.join(self.work_dir, 'package.zip')
        rocrate.download_to(zip_path)
        ...
        assert_true(
            'dateCreated' in _find_entity_by_id(json_entities, '#node1')
        )
        assert_true(
            'dateModified' in _find_entity_by_id(json_entities, '#node1')
        )
        assert_true('name' in _find_entity_by_id(json_entities, '#node1'))
        assert_equals(
            _find_entity_by_id(json_entities, '#node1-osfstorage'),
            {
                '@id': '#node1-osfstorage',
                '@type': 'RDMAddon',
                'about': {'@id': '#node1'},
    ...
    def test_simple_extraction(self):
        ...
        with mock.patch.object(ROCrateExtractor, '_download') as mock_request:
            ...
            new_wb, new_node_wb, new_wb_upload_file = (
                _create_waterbutler_client_for_single_node(new_node, {})
            )
            ...
            assert_equals(
                [
                    t.name
                    for t in new_node.get_addon('osfstorage')
                    .get_root()
                    .find_child_by_name('file_in_folder')
                    .tags.all()
                ],
                ['Test File'],
            )
            ...
            assert_equals(
                new_node.get_addon('metadata').get_file_metadata_for_path(
                    'osfstorage/file_in_root'
                )['items'],
                [
                    {
            ...
            new_node_wb.assert_has_calls(
                [
                    mock.call.get_file_by_materialized_path(
                        'osfstorage/sample/', create=True
                    ),
                    mock.call.get_file_by_materialized_path(
                        'osfstorage/sample/sub_folder/', create=True
                    ),
                    mock.call.get_file_by_materialized_path(
                        'osfstorage/sample/empty/', create=True
                    ),
                    mock.call.get_file_by_materialized_path(
                        'osfstorage/sample/'
                    ),
            ...
            new_wb_upload_file.assert_has_calls(
                [
                    mock.call(
                        'osfstorage/sample/', 'file_in_folder', b'FOLDER_DATA'
                    ),
                    mock.call(
                        'osfstorage/sample/sub_folder/',
                        'file_in_sub_folder',
    ...
    def test_composite_extraction(self):
        ...
        with mock.patch.object(ROCrateExtractor, '_download') as mock_request:
            ...
            _, new_node_wb, new_wb_upload_file = (
                _create_waterbutler_client_for_single_node(new_node, {})
            )
            _, new_child_node_wb, new_child_wb_upload_file = (
                _create_waterbutler_client_for_single_node(new_child, {})
            )
            new_wb = mock.MagicMock()
            new_wb.get_client_for_node = mock.MagicMock(
                side_effect=lambda n: new_node_wb
                if n == new_node
                else new_child_node_wb
            )
            extractor.ensure_folders(new_wb)
            for file_extractor in extractor.file_extractors:
                file_extractor.extract(new_wb)
            ...
            assert_equals(
                [t.name for t in new_node.tags.all()], ['Composite Node']
            )
