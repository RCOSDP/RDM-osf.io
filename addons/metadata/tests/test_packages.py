...
def _create_waterbutler_object(provider, file):
    ...
    def _get_files():
        ...
        return [_create_waterbutler_object(provider, file) for file in file['children']]
...
class TestExportAndImport(OsfTestCase):
    ...
    def test_files_only(self):
        ...
        assert_equals(
            remove_fields(_find_entity_by_id(json_entities, './'), fields=['datePublished']),
            {
                '@id': './',
                '@type': 'Dataset',
                'hasPart': [
        ...
        assert_equals(
            _find_entity_by_id(json_entities, '#creator0'),
            {
                '@id': '#creator0',
                '@type': 'Person',
                'familyName': [{'@language': 'en', '@value': creator.family_name}],
                'givenName': [{'@language': 'en', '@value': creator.given_name}],
                'identifier': [],
    ...
    def test_comments_and_files_only(self):
        ...
        _assert_dict_matches(
            _find_entity_by_id(json_entities, '#creator0'),
            {
                '@id': '#creator0',
                '@type': 'Person',
                'familyName': [{'@language': 'en', '@value': re.compile(r'Mercury[0-9]+')}],
    ...
    def test_wiki_only(self):
        ...
        assert_equals(
            remove_fields(_find_entity_by_id(json_entities, './'), fields=['datePublished']),
            {'@id': './', '@type': 'Dataset', 'hasPart': [{'@id': 'root/wiki/test'}]},
        )
        assert_true('datePublished' in _find_entity_by_id(json_entities, './'))
        assert_equals(
            _find_entity_by_id(json_entities, 'ro-crate-metadata.json'),
    ...
    def test_composite_extraction(self):
        ...
        with mock.patch.object(ROCrateExtractor, '_download') as mock_request:
            ...
            new_wb.get_client_for_node = mock.MagicMock(
                side_effect=lambda n: new_node_wb if n == new_node else new_child_node_wb
            )
            extractor.ensure_folders(new_wb)
            for file_extractor in extractor.file_extractors:
                file_extractor.extract(new_wb)
            ...
            assert_equals(
                new_node.get_addon('metadata').get_file_metadata_for_path('osfstorage/file_in_composite_root')[
                    'items'
                ],
                [
                    {
