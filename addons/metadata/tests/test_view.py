...
class TestViews(BaseAddonTestCase, OsfTestCase):
    ...
    def test_no_file_metadata(self):
        ...
        assert_true(
            'dataset_importing' in res.json['data']['attributes']['features']
        )
    ...
    def test_single_file_metadata(self):
        ...
        res = self.app.get(url, auth=self.user.auth)
        assert_equals(
            res.json['data']['attributes']['files'],
            [
                {
                    'path': 'osfstorage/',
                    'generated': False,
                    'hash': '1234567890',
                    'urlpath': '/{}/files/dir/osfstorage/'.format(
...
class TestSuggestionsViews(BaseAddonTestCase, OsfTestCase):
    ...
    def test_no_key(self):
        url = self.project.api_url_for(
            '{}_file_metadata_suggestions'.format(SHORT_NAME), filepath='fake'
        )
    ...
    def test_dir_with_multiple_keys(self, mock_get_metadata_assets):
        ...
        url = self.project.api_url_for(
            '{}_file_metadata_suggestions'.format(SHORT_NAME),
            filepath='dir/osfstorage/dir1/',
        )
        res = self.app.get(
            url,
            auth=self.user.auth,
            params={'key[]': ['file-data-number', 'asset:title']},
        )
                ...
                'data': {
                    ...
                    'attributes': {
                        ...
                        'suggestions': [
                            ...
                            {'key': 'asset:title', 'value': {'title': 'pine'}},
                            {
                                'key': 'asset:title',
    ...
    def test_file_with_multiple_keys(self, mock_get_metadata_assets):
        ...
        with mock.patch.object(
            BaseFileNode, 'resolve_class', return_value=mock_resolved_class
        ):
            url = self.project.api_url_for(
                '{}_file_metadata_suggestions'.format(SHORT_NAME),
                filepath=filepath,
    ...
    def test_asset_title_with_keyword(self, mock_get_metadata_assets):
        ...
        res = self.app.get(
            url,
            params={'key': 'asset:title', 'keyword': 'app'},
            auth=self.user.auth,
        )
        assert_equals(res.status_code, http_status.HTTP_200_OK)
        assert_equals(
