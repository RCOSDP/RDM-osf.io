...
def suggestion_erad(key, keyword, node):
    ...
    if isinstance(filter_field, EncryptedTextField) or isinstance(filter_field, EncryptedJSONField):
        ...
        candidates = [r for r in _erad_candidates_for_node(node) if keyword.lower() in r[filter_field_name].lower()]
    else:
...
def suggestion_contributor(key, keyword, node):
    ...
    if search_key == 'erad':
        contributors = [cont for cont in contributors if keyword in cont['erad']]
