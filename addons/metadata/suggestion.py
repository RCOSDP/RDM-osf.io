...
def suggestion_metadata(key, keyword, filepath, node):
    ...
    if key == 'file-data-number':
        ...
        suggestions.extend(
            suggestion_file_metadata_get_value(key, keyword, filepath, node)
        )
    elif key == 'ror':
...
def suggestion_file_metadata_get_value(key, keyword, filepath, node):
    ...
    try:
        ...
        if (
            key == 'get-excel-row-count' or key == 'get-excel-column-count'
        ) and extension in AddonAppConfig.excel_file_extension:
            ...
            key == 'get-text-row-count'
            and extension in AddonAppConfig.text_file_extension
        ):
            ...
            if file_size < text_file_maximum_size:
                ...
                try:
                    ...
                    delimiter = detect_delimiter(
                        content, AddonAppConfig.delimiters
                    )
            ...
            if file_size < text_file_maximum_size:
                ...
                try:
                    ...
                    delimiter_counts = {
                        delimiter: 0 for delimiter in delimiters
                    }
            ...
            key == 'get-image-type'
            and extension in AddonAppConfig.image_file_extension
        ):
            ...
            key == 'get-image-color-information'
            and extension in AddonAppConfig.image_file_extension
        ):
            if file_size < image_file_maximum_size:
                response = download_file(url, cookies)
        ...
        elif key == 'get-image-text/binary' and (
            extension in AddonAppConfig.text_file_extension
            or extension in AddonAppConfig.excel_file_extension
            or extension in AddonAppConfig.image_file_extension
        ):
...
def is_binary(file_content, extension):
    ...
    if m is None:
        ...
        if (
            ...
            or m.startswith('application/vnd.ms-excel')
        ):
            ret = True
...
def suggestion_erad(key, keyword, node):
    ...
    if isinstance(filter_field, EncryptedTextField) or isinstance(
        filter_field, EncryptedJSONField
    ):
        ...
        candidates = _erad_candidates_for_node(
            node, **{f'{filter_field_name}__icontains': keyword}
        )
    res = []
        ...
        res.append(
            {
                ...
                'value': {
                    **candidate,
                    ...
                    'kenkyusha_shimei_ja_msfullname': _to_msfullname(
                        kenkyusha_shimei_ja, 'ja'
                    ),
                    'kenkyusha_shimei_en_msfullname': _to_msfullname(
                        kenkyusha_shimei_en, 'en'
                    ),
...
def _erad_candidates(**pred):
    return [
        dict([(k.lower(), getattr(record, k.lower())) for k in ERAD_COLUMNS])
...
def suggestion_contributor(key, keyword, node):
    contributors = [
        {
            ...
            'name-ja-msfullname': _to_msfullname(
                _contributor_to_name_ja(user), 'ja'
            ),
            'name-en-msfullname': _to_msfullname(
                _contributor_to_name_en(user), 'en'
            ),
        }
        for user in node.contributors
    ]
    ...
    if search_key == 'erad':
        contributors = [
            cont for cont in contributors if keyword in cont['erad']
        ]
        ...
        raise KeyError('Invalid key: {}'.format(key))
    return [{'key': key, 'value': cont} for cont in contributors]
