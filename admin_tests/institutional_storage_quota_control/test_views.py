...
class TestUpdateQuotaUserListByInstitutionStorageID(AdminTestCase):
    ...
    def test__user(self):
        max_quota = 50
        request = RequestFactory().post(
            reverse(self.view_name, kwargs={'institution_id': self.institution01.id}), {'maxQuota': max_quota}
        )
    ...
    def test__superuser(self):
        ...
        region1.save()
        request = RequestFactory().post(
            reverse(self.view_name, kwargs={'institution_id': self.institution01.id}), {'maxQuota': new_max_quota}
        )
        request.user = self.superuser
        response = self.view(request, institution_id=self.institution01.id)

        nt.assert_equal(response.status_code, 302)
        user_quota = UserQuota.objects.filter(user=self.user1, storage_type=UserQuota.CUSTOM_STORAGE).first()
    ...
    def test_post_update_quota(self):
        ...
        region2.save()
        request = RequestFactory().post(
            reverse(self.view_name, kwargs={'institution_id': self.institution02.id}), {'maxQuota': new_max_quota}
        )
        request.user = self.superuser
    ...
    def test_post__institution_not_found(self):
        new_max_quota = 120
        request = RequestFactory().post(
            reverse(self.view_name, kwargs={'institution_id': 0}), {'maxQuota': new_max_quota}
        )
        request.user = self.institution01_admin
    ...
    def test_post__max_quota_value_is_none(self):
        ...
        region.save()
        request = RequestFactory().post(reverse(self.view_name, kwargs={'institution_id': self.institution01.id}), {})
        request.user = self.institution01_admin
        response = self.view(request, institution_id=self.institution01.id)
        nt.assert_equal(response.status_code, 302)
...
class TestUserListByInstitutionStorageID(AdminTestCase):
    ...
    def test__user(self):
        request = RequestFactory().get(reverse(self.view_name, kwargs={'institution_id': self.institution01.id}))
        request.user = self.user
    ...
    def test__superuser(self):
        ...
        # access inst02
        request = RequestFactory().get(reverse(self.view_name, kwargs={'institution_id': self.institution02.id}))
        request.user = self.superuser
        response = self.view(request, institution_id=self.institution02.id)
        nt.assert_equal(response.status_code, 200)

        # access non-existent institution
        request = RequestFactory().get(reverse(self.view_name, kwargs={'institution_id': 0}))
        request.user = self.superuser
    ...
    def test_get_userlist(self):
        ...
        view = setup_view(self.view_instance, request, institution_id=self.institution01.id)
    ...
    def test_get_institution__not_found(self):
        ...
        request = RequestFactory().get(reverse(self.view_name, kwargs={'institution_id': institution.id}))
        request.user = self.superuser

        view = setup_view(self.view_instance, request, institution_id=institution.id)
        with nt.assert_raises(Http404):
        ...
        view = setup_view(self.view_instance, request, institution_id=0)
        with nt.assert_raises(Http404):
            view.get_institution()
    ...
    def test__institution_id_not_valid(self):
        with self.assertRaises(NoReverseMatch):
            RequestFactory().get(reverse(self.view_name, kwargs={'institution_id': 'fake_id'}))
...
class TestInstitutionStorageListByAdmin(AdminTestCase):
    ...
    def test_get_redirect_to_user_list(self):
        request = RequestFactory().get(reverse('institutional_storage_quota_control:list_institution_storage'))
        request.user = self.user
        ...
        nt.assert_equal(
            response.url,
            '/institutional_storage_quota_control' '/user_list_by_institution_id/{}/'.format(self.institution.id),
        )
    ...
    def test_get_render_response(self):
        ...
        nt.assert_is_instance(response.context_data['view'], views.InstitutionStorageList)
