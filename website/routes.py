...
def set_status_message(user):
    if user and not user.accepted_terms_of_service:
        status.push_status_message(
            message=_(language.TERMS_OF_SERVICE_SENTENCE).format(
                api_domain=settings.API_DOMAIN, user_id=user._id, csrf_token=json.dumps(g.get('csrf_token'))
            )
            + _(language.HAVE_READ_AGREE).format(
                api_domain=settings.API_DOMAIN, user_id=user._id, csrf_token=json.dumps(g.get('csrf_token'))
            )
            + _(language.CONTINUE).format(
                api_domain=settings.API_DOMAIN, user_id=user._id, csrf_token=json.dumps(g.get('csrf_token'))
            )
            + language.TERMS_OF_SERVICE_SCRIPT.format(
                api_domain=settings.API_DOMAIN, user_id=user._id, csrf_token=json.dumps(g.get('csrf_token'))
            ),
            kind='default',
            dismissible=True,
...
def get_globals():
    ...
    return {
        ...
        'mfa_url': settings.CAS_SERVER_URL + '/logout?mfa_url=' + settings.OSF_MFA_URL,  # R-2023-55
...
def favicon():
    return send_from_directory(settings.STATIC_FOLDER, 'favicon.ico', mimetype='image/vnd.microsoft.icon')
...
def sitemap_file(path):
    ...
    return send_from_directory(settings.STATIC_FOLDER + '/sitemaps/', path, mimetype=mime)


def firebase():
    if os.path.exists(os.path.join(settings.STATIC_FOLDER, 'js/firebase-messaging-sw.js')):
        firebase_file = 'js/firebase-messaging-sw.js'
    else:
        raise HTTPError(http_status.HTTP_404_NOT_FOUND)
    return send_from_directory(settings.STATIC_FOLDER, firebase_file, mimetype='text/javascript')
...
def make_url_map(app):
    ...
    process_rules(
        ...
        [
            ...
            Rule('/api/v1/<path:_>', ['get', 'post'], HTTPError(http_status.HTTP_404_NOT_FOUND), json_renderer),
        ],
    )
    ...
    if settings.USE_EXTERNAL_EMBER:
        ...
        if EXTERNAL_EMBER_APPS.get('ember_osf_web'):
            process_rules(
                app, [Rule(ember_osf_web_views.routes, 'get', ember_osf_web_views.use_ember_app, notemplate)]
            )
    ...
    process_rules(
        ...
        [
            ...
            Rule('/myprojects/', 'get', website_views.my_projects, OsfWebRenderer('my_projects.mako', trust=False)),
            ...
            Rule('/view/<meeting>/', 'get', conference_views.redirect_to_conference_results, notemplate),
            Rule(
                '/view/<meeting>/plain/',
                'get',
                conference_views.conference_results,
            ...
            Rule(
                '/prereg/', 'get', prereg.prereg_landing_page, OsfWebRenderer('prereg_landing_page.mako', trust=False)
            ),
            Rule(
                '/preprints/',
                'get',
    ...
    process_rules(
        ...
        [
            # mapcore OAuth start
            Rule(
                '/mapcore_oauth_start', 'get', mapcore_oauth_start, OsfWebRenderer('mapcore_agree.mako', trust=False)
            ),
            # mapcore Oauth result catcher
            Rule('/mapcore_oauth_complete', 'get', mapcore_oauth_complete, notemplate),
        ],
    )
    ...
    process_rules(
        ...
        [
            # confirm email
            Rule('/confirm/<uid>/<token>/', 'get', auth_views.confirm_email_get, notemplate),
            # confirm email for login through external identity provider
            Rule('/confirm/external/<uid>/<token>/', 'get', auth_views.external_login_confirm_email_get, notemplate),
            # reset password get
            Rule(
            ...
            # user sign up page
            Rule('/register/', 'get', auth_views.auth_register, OsfWebRenderer('public/register.mako', trust=False)),
            ...
            Rule('/login/connected_tools/', 'get', landing_page_views.connected_tools, notemplate),
            Rule('/login/enriched_profile/', 'get', landing_page_views.enriched_profile, notemplate),
        ],
    )
    ...
    process_rules(
        app,
        [
            Rule('/profile/', 'get', profile_views.profile_view, OsfWebRenderer('profile.mako', trust=False)),
            Rule('/profile/<uid>/', 'get', profile_views.profile_view_id, OsfWebRenderer('profile.mako', trust=False)),
    ...
    if settings.DEBUG_MODE is True:
        process_rules(
            app,
            [Rule('/search_cos/', 'get', search_views.search_view_cos, OsfWebRenderer('search.mako', trust=False))],
        )
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                ['/search_raw/', '/search_raw/<type>/'], ['get', 'post'], search_views.search_search_raw, json_renderer
            ),
    ...
    process_rules(
        ...
        [
            ...
            # Invite Users
            Rule(
                ['/project/<pid>/invite_contributor/', '/project/<pid>/node/<nid>/invite_contributor/'],
                'post',
