...
describe('fangorn', () => {
    ...
    describe('doItemOp', () => {
        ...
        // _rejectConflictItem isn't exported, so these two exercise it through its only
        // real entry points (the oversized-file and quota pre-flight checks in doItemOp)
        // to cover the object-identity pendingReadyFiles filter and its doSyncMove advance.
        it('keeps other pending files queued when a pre-flight reject leaves the sync queue non-empty', () => {
            var tb = makeTb();
            var to = makeItem('folder', {provider: 's3compat', nodeApiUrl: 'https://osf.io/to/', accept: {maxSize: 1}}, 5, 1);
            var from = makeItem('file', {name: 'big.txt', size: 5000000}, 3, 1);
            ...
            ajaxStub = sinon.stub($, 'ajax');
            Fangorn.doItemOp.call(tb, Fangorn.OPERATIONS.MOVE, to, from, undefined, 'replace');
            // Filtered by object identity, not id: "other" survives untouched.
            assert.deepEqual(tb.pendingReadyFiles, [other]);
            assert.equal(ajaxStub.called, false);
            // Queue still has "other" pending, so doSyncMove must not have run yet.
            assert.equal(tb.modal.dismiss.callCount, 1);
            assert.equal(tb.modal.update.called, false);
        });

        it('advances the sync move queue via doSyncMove once a pre-flight reject empties pendingReadyFiles', () => {
            var tb = makeTb();
            var to = makeItem('folder', {provider: 's3compat', nodeApiUrl: 'https://osf.io/to/', accept: {maxSize: 1}}, 5, 1);
            var from = makeItem('file', {name: 'big.txt', size: 5000000}, 3, 1);
            tb.pendingReadyFiles = [from];
            ajaxStub = sinon.stub($, 'ajax');
            Fangorn.doItemOp.call(tb, Fangorn.OPERATIONS.MOVE, to, from, undefined, 'replace');
            assert.equal(tb.pendingReadyFiles.length, 0);
            // Emptying the queue must trigger doSyncMove -> displayMoveStats, which shows
            // the move-status modal because addFileStatus already recorded the rejection
            // (displayMoveStats resets tb.moveStates back to [] once it builds the modal).
