...
urlpatterns = [
    ...
    url(
        r'^institutional_storage/institutions/$',
        views.InstitutionalStorageListView.as_view(),
        name='institutional_storage_institutions',
    ),
    url(
        r'^institutional_storage/institutions/(?P<institution_id>[0-9]+)/$',
        views.InstitutionalStorageView.as_view(),
        name='institutional_storage_list',
    ),
    ...
    url(
        r'^save_credentials/(?P<institution_id>[0-9]+)$', views.SaveCredentialsView.as_view(), name='save_credentials'
    ),
    url(r'^credentials/(?P<institution_id>[0-9]+)$', views.FetchCredentialsView.as_view(), name='credentials'),
    url(
        r'^fetch_temporary_token/(?P<institution_id>[0-9]+)$',
        views.FetchTemporaryTokenView.as_view(),
        name='fetch_temporary_token',
    ),
    url(
        r'^remove_auth_data_temporary/(?P<institution_id>[0-9]+)$',
        views.RemoveTemporaryAuthData.as_view(),
        name='remove_auth_data_temporary',
    ),
